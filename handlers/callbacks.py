...
async def edit_or_answer(c, text, kb=None, parse="HTML"):
    try: 
        await c.message.edit_text(text, reply_markup=kb, parse_mode=parse)
...
def get_emoji(item):
    emoji_map = {
        "двенашка": "🧱", "атмосфера": "🌀", "энергетик": "⚡", "перчатки": "🧤",
...
class IgnoreNotModifiedMiddleware(BaseMiddleware):
    async def __call__(self, handler, event, data):
        try: 
            ...
            error_str = str(e)
            if "message is not modified" in error_str or ("Bad Request" in error_str and "exactly the same" in error_str):
...
async def mm_text(p):
    ...
    return (f"<b>Главное меню</b>\n{rank_emoji} <b>{rank_name}</b> | ⭐ {p.get('avtoritet',1)} | 📈 Ур. {p.get('level',1)}\n\n"
            ...
            f"💸 Деньги: {p.get('dengi',0)}р | 🐍 Змий: {p.get('zmiy',0):.1f}кг\n\n"
...
async def handle_act(c, act):
    if not (h := ACTION_HANDLERS.get(act)): return
    ...
    if act == "davka":
        up = p.get("upgrades",{})
        ex["nm"] = "\n🥛 Ряженка жмёт двенашку как надо! (+75%)" if up.get("ryazhenka") else "\n🧋 Бублэки создают нужную турбулентность! (+35% к шансу)" if up.get("bubbleki") else ""
...
async def cb_action(c): 
    await handle_act(c, c.data)
...
async def cb_inventory(c):
    p, inv, ab = await get_patsan_cached(c.from_user.id), p.get("inventory",[]), p.get("active_boosts",{})
    if not inv: 
        t = "Пусто... Только пыль и тоска"
    else:
        cnt = {i: inv.count(i) for i in set(inv)}
        t = "<b>Твои вещи:</b>\n" + "\n".join(f"{get_emoji(i)} {i}: {cnt} шт." for i, cnt in cnt.items())
    if ab:
        t += "\n\n<b>🔮 Активные бусты:</b>\n"
        for b,e in ab.items():
...
async def cb_profile(c):
    p, re, rn = await get_patsan_cached(c.from_user.id), *get_user_rank(p)
    ac, ma, up = p.get('atm_count',0), p.get('max_atm',12), p.get("upgrades",{})
    bu, sp, sb = [k for k,v in up.items() if v] if up else [], p.get("specialization"), get_specialization_bonuses(sp) if p.get("specialization") else {}
    await edit_or_answer(c, 
        ...
        + (f"\n<b>🌳 Специализация:</b> {sp}\n<i>Бонусы: {', '.join(sb.keys())}</i>" if sp else ""), 
...
async def cb_specs(c):
    ...
    for s in av:
        ...
        t += "\n"
    await edit_or_answer(c, t + "<i>Выбери специализацию для подробной информации:</i>", specializations_keyboard())
...
async def cb_spec_detail(c):
    ...
    if st == "info":
        ...
        return
    if st not in SPECS: return await c.answer("Неизвестная специализация", show_alert=True)
    s = SPECS[st]
...
async def cb_craft_items(c):
    ...
    for i in ci:
        ...
        t += "\n"
    await edit_or_answer(c, t + "<i>Выбери предмет для крафта:</i>", craft_items_keyboard())
...
async def cb_scout_menu(c):
    p, su, fl = await get_patsan_cached(c.from_user.id), p.get("rademka_scouts",0), max(0,5-p.get("rademka_scouts",0))
...
async def cb_scout_random(c):
    ...
    if not targets:
        ...
        return
    t, ok, msg, sd = random.choice(targets), *await rademka_scout(uid, t.get("user_id"))
...
async def cb_scout_target(c):
    ...
    if d == "choose":
        ...
        p, su, fu = await get_patsan_cached(c.from_user.id), p.get("rademka_scouts",0), min(5,p.get("rademka_scouts",0))
...
ACHS = {
    "zmiy_collector": {"n": "Коллекционер змия", "d": "Собери определённое количество змия", "l": [{"g": 10, "r": 50, "t": "Новичок", "e": 10}, {"g": 100, "r": 300, "t": "Любитель", "e": 50}, {"g": 1000, "r": 1500, "t": "Профессионал", "e": 200}, {"g": 10000, "r": 5000, "t": "КОРОЛЬ ГОФРОЦЕНТРАЛА", "e": 1000}]},
    "money_maker": {"n": "Денежный мешок", "d": "Заработай много денег", "l": [{"g": 1000, "r": 100, "t": "Бедолага", "e": 10}, {"g": 10000, "r": 1000, "t": "Состоятельный", "e": 100}, {"g": 100000, "r": 5000, "t": "Олигарх", "e": 500}, {"g": 1000000, "r": 25000, "t": "РОТШИЛЬД", "e": 2500}]},
    "rademka_king": {"n": "Король радёмок", "d": "Победи в множестве радёмок", "l": [{"g": 5, "r": 200, "t": "Задира", "e": 20}, {"g": 25, "r": 1000, "t": "Гроза района", "e": 100}, {"g": 100, "r": 5000, "t": "Неприкасаемый", "e": 500}, {"g": 500, "r": 25000, "t": "ЛЕГЕНДА РАДЁМКИ", "e": 2500}]}
}
...
async def cb_ach_progress(c):
    ...
    for aid, d in pd.items():
        ...
        t += "\n"
    await edit_or_answer(c, t + "<i>Выбери достижение для подробной информации:</i>", achievements_progress_keyboard())

@router.callback_query(F.data.startswith("achievement_"))
async def cb_ach_detail(c):
    if (at := c.data.replace("achievement_","")) not in ACHS: return await c.answer("Неизвестное достижение", show_alert=True)
    a = ACHS[at]
...
async def cb_level_stats(c):
    p, cl, ce = await get_patsan_cached(c.from_user.id), p.get("level",1), p.get("experience",0)
...
async def cb_atm_status(c):
    p, ac, ma = await get_patsan_cached(c.from_user.id), p.get('atm_count',0), p.get('max_atm',12)
...
TOPS = {"avtoritet": ("авторитету", "⭐", "avtoritet"), "dengi": ("деньгам", "💰", "dengi"), "zmiy": ("змию", "🐍", "zmiy"), "total_skill": ("сумме скиллов", "💪", "total_skill"), "level": ("уровню", "📈", "level"), "rademka_wins": ("победам в радёмках", "👊", "rademka_wins")}
...
async def get_rademka_winners_top():
    ...
    await cn.close()
    return [dict(r) | {"wins": r["wins"] or 0, "rank": "?", "zmiy": 0, "dengi": 0, "level": 1} for r in rows]
...
async def cb_show_top(c):
    ...
    sn, em, dk = TOPS[st]
    try:
        tp = await get_top_players(limit=10, sort_by=dk) if st != "rademka_wins" else await get_rademka_winners_top()
    ...
    for i, pl in enumerate(tp[:10]):
        ...
        rank_val = pl.get('rank','').split(' ')
        ri = f" ({rank_val[1]})" if len(rank_val) > 1 and st != "rademka_wins" else ""
...
async def cb_inv_action(c):
    ...
    elif a == "sort": await c.answer("Инвентарь отсортирован!", show_alert=True); await cb_inventory(c)
    elif a == "trash": await edit_or_answer(c, "🗑️ <b>ВЫБРОСИТЬ МУСОР</b>\n\nТы уверен? Это действие удалит:\n• Все 'перчатки'\n• Все 'швабры'\n• Все 'вёдра'\n\nЗато освободит место в инвентаре!", confirmation_keyboard("trash_inventory"))
...
async def cb_confirm_trash(c):
    p, inv = await get_patsan(c.from_user.id), p.get("inventory",[])
    new = [i for i in inv if i not in ["перчатки", "швабра", "ведро"]]
    if (r := len(inv) - len(new)) > 0:
        p["inventory"] = new
        await save_patsan(p)
