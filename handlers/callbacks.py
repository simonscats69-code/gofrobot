...
async def nickname_menu(c):
    try:
        ...
        await edit_or_answer(c, 
            ...
            f"📝 <b>Твой ник:</b> <code>{p.get('nickname','Неизвестно')}</code>\n"
            f"⭐ <b>Авторитет:</b> {p.get('avtoritet',1)}\n"
...
async def cb_davka(c): 
    await handle_act(c, "davka")

@router.callback_query(F.data == "sdat")
async def cb_sdat(c): 
    await handle_act(c, "sdat")
...
async def cb_pump(c):
    ...
    await edit_or_answer(c, 
        ...
        f"📈 Уровень: {p.get('level',1)} | 📚 Опыт: {p.get('experience',0)}\n\n"
...
async def cb_inventory(c):
    ...
    await edit_or_answer(c, 
        ...
        f"🔨 Скрафчено предметов: {len(p.get('crafted_items',[]))}", 
        inventory_management_keyboard())
...
async def cb_profile(c):
    ...
    await edit_or_answer(c, 
        ...
        f"👤 {p.get('nickname','Неизвестно')}\n"
        f"⭐ Авторитет: {p.get('avtoritet',1)}\n"
        f"📈 Уровень: {p.get('level',1)} | 📚 Опыт: {p.get('experience',0)}\n\n"
        f"<b>Ресурсы:</b>\n🌀 Атмосферы: [{pb(ac,ma)}] {ac}/{ma}\n"
        f"⏱️ Восстановление: {ft(calculate_atm_regen_time(p))}\n"
        f"🐍 Коричневаг: {p.get('zmiy',0):.3f} кг\n💰 Деньги: {p.get('dengi',0)} руб.\n\n"
        f"<b>Скиллы:</b>\n💪 Давка: {p.get('skill_davka',1)}\n"
        f"🛡️ Защита: {p.get('skill_zashita',1)}\n🔍 Находка: {p.get('skill_nahodka',1)}"
        f"{ut}{st}", 
        profile_extended_keyboard())

SPECS = {
    "davila": {
        "n": "Давила", "d": "Мастер давления коричневага", 
        "r": "💪 Давка змия: 5 ур.\n🐍 Накоплено змия: 50кг",
        "b": "• +50% к выходу змия\n• -1 атмосфера на действие\n• Открывает: Гигантская давка", 
        "p": 1500
    },
    "ohotnik": {
        "n": "Охотник за двенашками", "d": "Находит то, что другие не видят",
        "r": "🔍 Находка двенашек: 5 ур.\n🧱 Двенашка в инвентаре",
        "b": "• +15% к шансу находок\n• 5% шанс на редкий предмет\n• Открывает: Детектор двенашек", 
        "p": 1200
    },
    "neprobivaemy": {
        "n": "Непробиваемый", "d": "Железные кишки и стальные нервы",
...
async def cb_specs(c):
    ...
    t += "<i>Специализация даёт уникальные бонусы и открывает новые возможности.</i>\n"
    t += "<i>Можно выбрать только одну. Выбор бесплатен при выполнении требований.</i>\n\n"
...
async def cb_spec_detail(c):
    ...
    await edit_or_answer(c, 
        ...
        specialization_confirmation_keyboard(st))
...
async def cb_craft(c):
    ...
    await edit_or_answer(c, 
        ...
        f"📦 Инвентарь: {len(p.get('inventory',[]))} предметов\n"
        f"🔨 Скрафчено: {len(p.get('crafted_items',[]))} предметов\n"
        f"💰 Деньги: {p.get('dengi',0)}р\n\n"
...
async def cb_scout_menu(c):
    ...
    await edit_or_answer(c, 
        ...
        f"• Использовано разведок: {su}\n• Бесплатных осталось: {fl}/5\n"
        f"• Стоимость разведки: {0 if fl>0 else 50}р\n\n"
        f"<i>Выбери действие:</i>", 
...
async def cb_scout_random(c):
    ...
    txt = (f"🎯 <b>РАЗВЕДКА ЗАВЕРШЕНА!</b>\n\n"
           ...
           f"• Его авторитет: {ts.get('avtoritet',0)} ({tr})\n"
           f"• Последняя активность: {ts.get('last_active_hours',0)}ч назад\n\n"
           f"💸 Стоимость разведки: {'Бесплатно' if sd.get('cost',0)==0 else '50р'}\n"
           f"🕵️ Бесплатных разведок осталось: {sd.get('free_scouts_left',0)}\n\n"
...
ACHS = {
    "zmiy_collector": {
        "n": "Коллекционер змия", "d": "Собери определённое количество змия",
        "l": [
    ...
    "money_maker": {
        "n": "Денежный мешок", "d": "Заработай много денег",
        "l": [
    ...
    "rademka_king": {
        "n": "Король радёмок", "d": "Победи в множестве радёмок",
        "l": [
...
async def cb_ach_progress(c):
    ...
    if not pd:
        await edit_or_answer(c, 
            ...
            achievements_progress_keyboard())
        return
    ...
    for aid, d in pd.items():
        ...
        if d['next_level']:
            ...
            t += f"Прогресс: {d['current_progress']:.1f}/{d['next_level']['goal']} ({d['progress_percent']:.1f}%)\n"
...
async def cb_level_stats(c):
    ...
    t = (f"<b>📈 СТАТИСТИКА УРОВНЕЙ</b>\n\n"
         f"🏆 <b>Текущий уровень:</b> {cl}\n"
         f"📚 <b>Опыт:</b> {ce}/{re}\n"
         f"📊 <b>Прогресс:</b> [{pb(ce,re,10)}] {pp:.1f}%\n\n"
         f"🎁 <b>Награда за {cl+1} уровень:</b>\n• +{nr}р\n")
    
    if ai: 
        t += "• +1 к максимальным атмосферам\n"
...
async def cb_atm_status(c):
    ...
    t = (f"<b>🌡️ СОСТОЯНИЕ АТМОСФЕР</b>\n\n"
         f"🌀 <b>Текущий запас:</b> {ac}/{ma}\n"
         f"📊 <b>Заполненность:</b> [{pb(ac,ma)}] {(ac/ma)*100:.1f}%\n\n"
         f"⏱️ <b>Время восстановления:</b>\n• 1 атмосфера: {ft(rt)}\n"
         f"• До полного: {ft(rt*(ma-ac))}\n\n")
    
    if bs:
        t += f"⚡ <b>Активные бонусы:</b>\n" + "\n".join(f"• {b}" for b in bs) + "\n\n"
...
async def cb_show_top(c):
    ...
    try:
        if st == "rademka_wins":
            ...
            cur = await cn.execute(
                'SELECT u.user_id,u.nickname,u.avtoritet,COUNT(rf.id) as wins '
                'FROM users u LEFT JOIN rademka_fights rf ON u.user_id=rf.winner_id '
                'GROUP BY u.user_id,u.nickname,u.avtoritet ORDER BY wins DESC LIMIT 10'
            )
...
async def cb_confirm_trash(c):
    ...
    if r > 0:
        ...
        await edit_or_answer(c, 
            ...
            f"Выброшено предметов: {r}\n"
            f"Осталось в инвентаре: {len(new)}\n\n"
