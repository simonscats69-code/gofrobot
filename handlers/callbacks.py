...
async def back_main(c): await edit_or_answer(c,await mm_text(await get_patsan_cached(c.from_user.id)),main_keyboard())
...
async def cb_inventory(c):
    ...
    if not i:t="Пусто... Только пыль и тоска"
        ...
        t="<b>Твои вещи:</b>\n"+"\n".join(f"{get_emoji(x)} {x}: {c} шт." for x,c in cnt.items())
    if ab:
        t+="\n\n<b>🔮 Активные бусты:</b>\n"
        for b,e in ab.items():
...
async def cb_spec_buy(c):
    ...
    if ok:await edit_or_answer(c,f"🎉 <b>ПОЗДРАВЛЯЮ!</b>\n\n{msg}\n\nТеперь ты обладатель уникальной специализации!\nИспользуй её бонусы по максимуму.",main_keyboard())
    else:await c.answer(msg,show_alert=True);await cb_specs(c)
