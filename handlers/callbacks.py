...
async def csp(c):
    try:
        ...
        if cs:
            ...
            bonuses_text = "\n".join(f"• {k}: {v}" for k, v in sb.items()) if sb else "• Нет информации о бонусах"
        ...
        if not av: t += "<i>Нет доступных специализаций. Выполните требования для их открытия.</i>"
        else:
            for s in av:
                if not isinstance(s, dict): continue
                name, description = s.get('name', 'Неизвестно'), s.get('description', 'Нет описания')
                available, price = s.get('available', False), s.get('price', 0)
